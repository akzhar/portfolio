...
gulp.task('sprite', function () { // задача - вызывается как скрипт из package.json
  return gulp
  .src('src/img/sprite/inline-*.svg') // источник
  .pipe(imagemin([
...
gulp.task('image', function () { // задача - вызывается как скрипт из package.json
  return gulp
  .src('src/img/*.{png,jpg,svg}') // источник
  .pipe(imagemin([
...
gulp.task('watch', function() { // задача - вызывается как скрипт из package.json
  ...
  gulp.watch('src/blocks/**/*.{scss,sass}', gulp.series('style')); // отслеживание изменений файлов scss

  gulp.watch('src/js/**/*.js', gulp.series('js')); // отслеживание изменений файлов js

  gulp.watch('src/blocks/**/*.html', gulp.series('html')); // отслеживание изменений файлов html

  gulp.watch('src/img/*.*', gulp.series('image')); // отслеживание изменений файлов img
  gulp.watch('src/img/sprite/inline-*.svg', gulp.series('sprite', 'html')); // отслеживание изменений файлов sprite svg
});
...
gulp.task ('serve', function(done) { //задача - вызывается как скрипт из package.json
  ...
  server.watch('docs/').on('change', server.reload);
  done();
});
